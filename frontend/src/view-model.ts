...
export function taskProgressText(thread: Thread | null, status: Status, visibleEventCount: number) {
  ...
  if (status.className === "running") return "正在执行，新的记录会自动同步";
...
export function materialEntries(events: TimelineEvent[], attachments: AttachmentDraft[]): MaterialEntry[] {
  const pending = attachments.map((attachment) => ({
    ...
    state: "待发送" as const,
    context: attachment.kind === "image" ? "发送时作为图片输入" : "发送时生成文件引用",
  ...
  const committed = attachmentEvents(events).map((item, index) => ({
    ...
    token: item.token,
    state: "已提交" as const,
    context: item.kind === "image" ? "图片内容已提交" : "文件以引用形式进入任务说明",
...
export function modelLevelDisplayLabel(value?: string) {
...
export function modelLevelOptions(capabilities: RemoteCapabilities | null, currentValue = "") {
  ...
  const labelForLevel = (level: ModelLevelSummary) => {
    const value = String(level.id || "");
    const model = String(level.model || "");
    const modelName = String(level.model_name || "");
    const fullModel = modelName && model && modelName !== model ? `${modelName} -> ${model}` : model || modelName;
    const label = levelLabel(value);
    return fullModel ? `${label} · ${fullModel}` : label;
  };
  const defaultItem = levels.find((level) => String(level.id || "") === defaultLevel);
  const options = levels.length
    ? [
        { value: "", label: defaultItem ? `默认 · ${labelForLevel(defaultItem)}` : defaultLevel ? `默认 · ${levelLabel(defaultLevel)}` : "默认" },
        ...levels.map((level) => {
          const value = String(level.id || "");
          return { value, label: labelForLevel(level) };
...
export function modelLevelById(capabilities: RemoteCapabilities | null, value = "") {
  const levels = capabilities?.core?.models?.levels || [];
  return levels.find((level) => String(level.id || "") === value) || null;
}
...
export function availablePickerGroups(pickers: Record<string, PickerSummary> = {}) {
  return Object.entries(pickers)
    .filter(([, picker]) => picker?.available && (picker.items || []).length)
    .map(([key, picker]) => ({ key, picker, items: mentionItems(picker) }))
...
export function pickerTitle(picker?: PickerSummary, fallback = "插件") {
  const title = picker?.title;
  if (typeof title === "string" && title.trim()) return title.trim();
  if (title && typeof title === "object") return String(title.zh || title.en || Object.values(title)[0] || fallback);
  return fallback;
}

export function commandInsertText(command: CommandItemSummary) {
